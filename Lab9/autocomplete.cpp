...
{
	int curr_index = binary_search(key);
	if(curr_index == -1){
		firstIndex = -1;
		lastIndex = -1;
		return;}
	...
	for(int i = curr_index; i >= 0; i--){
		if(terms[i].compareByPrefix(terms[i], terms[curr_index], key.length())!=0){
			firstIndex = i+1;
			break;
		}
	...
	for(int j = curr_index; j < terms.size(); j++){
		if(terms[j].compareByPrefix(terms[j], terms[curr_index], key.length())!=0){
			lastIndex = j-1;
			break;
		}
